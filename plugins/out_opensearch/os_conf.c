                     ...
                     struct flb_config *config)
{
    ...
    if (oc->compression_str) {
        if (strcasecmp(oc->compression_str, "gzip") == 0) {
            oc->compression = FLB_OS_COMPRESSION_GZIP;
        }
        else {
            oc->compression = FLB_OS_COMPRESSION_NONE;
        }
    }
    else {
        oc->compression = FLB_OS_COMPRESSION_NONE;
    ...
    oc->aws_unsigned_headers = flb_malloc(sizeof(struct mk_list));
    if (!oc->aws_unsigned_headers) {
        flb_os_conf_destroy(oc);
        return -1;
    }
    flb_slist_create(oc->aws_unsigned_headers);
    ret = flb_slist_add(oc->aws_unsigned_headers, "Content-Length");
    if (ret != 0) {
        flb_os_conf_destroy(oc);
        return -1;
    }
...
int flb_os_conf_destroy(struct flb_opensearch_config *oc)
{
    ...
    if (oc->aws_unsigned_headers) {
        flb_slist_destroy(oc->aws_unsigned_headers);
        flb_free(oc->aws_unsigned_headers);
    }
#endif

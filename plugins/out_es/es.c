...
*/
static flb_sds_t es_get_id_value(struct flb_elasticsearch_config *ec,
                                 struct flb_output_instance *ins,
                                 msgpack_object *map)
{
                                ...
                                void **out_data, size_t *out_size)
{
    ...
    struct flb_elasticsearch_config *ec = plugin_context;
    struct flb_elasticsearch *ctx = flush_ctx;
    ...
    while (msgpack_unpack_next(&result, data, bytes, &off) == MSGPACK_UNPACK_SUCCESS) {
        ...
        if (ec->ra_id_key) {
            id_key_str = es_get_id_value(ec , ctx->ins, &map);
            if (id_key_str) {
                if (ec->suppress_type_name) {
                    index_len = snprintf(j_index,
                                         ES_BULK_HEADER,
                      ...
                      void *data)
{
    ...
    if (tmp) {
        ...
        ret = es_config_simple(ctx, ins, config);
    }
                        ...
                        struct flb_config *config)
{
    ...
    if (ec->has_aws_auth == FLB_TRUE) {
        signature = add_aws_auth(c, ctx->ins, ec);
        if (!signature) {
            goto retry;
        }
    }
